...
//비회원
if ( empty($user_id) ) {
?>    <style> #reg_btn, #logout_btn {display:none !important;} </style>
<?  $user_id = "비회원";
} else {
?>
    <style> #sign_btn {display:none !important;} </style>
<?
}
...
<body>
    <div id="user_area">
        <b><?= $user_id ?></b>님 환영합니다!
        <button type="button" onClick="logoutCheck();" id="logout_btn" style="width: 80px; height:30px; margin-left:860px;">로그아웃</button>
    </div>
    <div id="board_area">
        <a href="/bbs_sign.php"><button type="button" id="sign_btn" style="width: 80px; height:30px; margin-left:860px;">회원가입</button></a>
        <h1>자유게시판</h1>
        ...
        <table class="list-table">
            ...
            <? while ($tbl_bbs = mysqli_fetch_array($sql_list)) { ?>
                <tbody>
                    <tr>
                        <td><?= $tbl_bbs['no'] ?></td>
                        <td><a href="/bbs_content.php?page=<?= $pno ?>&no=<?= $tbl_bbs['no'] ?>&sort=<?= $sort ?>"><?= $tbl_bbs['title'] ?></a></td>
